...
class GaussianProcess(object):
    ...
    def get_gp_mean_std(self,
        ...
        all the points in data_points
        """
        n,l=new_data_points.shape

        X=GaussianProcess._array_dataset
        y=GaussianProcess._array_objective_function_values
        cov_matrix=GaussianProcess._covariance_matrix
        kernel=GaussianProcess._kernel



        mean=kernel(new_data_points,X) @ np.linalg.inv(cov_matrix+self._kernel.noise_scale_squared*np.eye(cov_matrix.shape(0))) @ y

        std=kernel(new_data_points,new_data_points)+self._kernel.noise_scale_squared-kernel(new_data_points,X) @ np.linalg.inv(cov_matrix+self._kernel.noise_scale_squared*np.eye(cov_matrix.shape(0))) @ kernel(X,new_data_points)
